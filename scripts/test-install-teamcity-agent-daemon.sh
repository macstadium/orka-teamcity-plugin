...
# What a VM that previously ran as an SSH-mode agent carries into the image.
printf 'oldInstanceId\noldImageId\n' > "${AGENT_DIR}/orka_metadata_file"
...
check "gives the agent the documented 60s to unregister before SIGKILL" \
    "$([ "$(plutil -extract ExitTimeOut raw -o - "$PLIST")" -eq 60 ] && echo true || echo false)"
check "wrapper is created and executable" \
    "$([ -x "$WRAPPER" ] && echo true || echo false)"
# Ownership cannot be asserted here: the dry-run hook skips chown along with root.
check "wrapper is not group or world writable" \
    "$([ "$(stat -f '%Lp' "$WRAPPER")" = "755" ] && echo true || echo false)"
check "plist is not group or world writable" \
    "$([ "$(stat -f '%Lp' "$PLIST")" = "644" ] && echo true || echo false)"
...
# Poll below counts lines in this file; without it grep has no file and prints no count at all.
: > "$CALL_LOG"
...
if [ "\$1" = "start" ]; then
    sleep 30 &
    echo \$! > "${STOP_AGENT_DIR}/logs/buildAgent.pid"
elif [ "\$1" = "stop" ]; then
    pid="\$(cat "${STOP_AGENT_DIR}/logs/buildAgent.pid")"
    # Acknowledge stop immediately, remove the PID file, but finish shutdown later.
    rm "${STOP_AGENT_DIR}/logs/buildAgent.pid"
    (sleep 1; touch "${STOP_WORK}/shutdown-complete"; kill "\$pid") &
fi
exit 0
STUB
...
# Stands in for an image captured from a VM that once ran in SSH mode. The agent-side plugin
# reads this file as its identity when it is there, so leaving it would make every clone of the
# image register under the captured VM's instance id.
printf 'stale-instance-id\nstale-image-id\n' > "${STOP_AGENT_DIR}/orka_metadata_file"
...
for _ in $(seq 1 200); do
    if [ "$(grep -c '^start$' "$CALL_LOG" 2>/dev/null || true)" -ge 2 ]; then
        started="true"
        break
    fi
    sleep 0.1
...
# Ordered before the first start, so the agent never sees the stale file even on a boot where it
# comes up immediately.
check "wrapper clears a stale identity file before starting the agent" \
    "$([ ! -f "${STOP_AGENT_DIR}/orka_metadata_file" ] && echo true || echo false)"
...
check "wrapper waits for asynchronous shutdown even after PID file removal" \
    "$([ -f "${STOP_WORK}/shutdown-complete" ] && echo true || echo false)"

# A stale PID file or an already-gone agent makes 'agent.sh stop force' fail. The wrapper must
# still exit 0, or KeepAlive.SuccessfulExit=false has launchd respawn the agent mid-shutdown.
FAILSTOP_WORK="${WORK_DIR}/failstop-test"
FAILSTOP_AGENT_DIR="${FAILSTOP_WORK}/BuildAgent"
FAILSTOP_LOG="${FAILSTOP_WORK}/calls.log"
mkdir -p "${FAILSTOP_AGENT_DIR}/bin" "${FAILSTOP_AGENT_DIR}/conf" "${FAILSTOP_WORK}/out"
cat > "${FAILSTOP_AGENT_DIR}/bin/agent.sh" <<STUB
#!/bin/sh
echo "\$@" >> "${FAILSTOP_LOG}"
if [ "\$1" = "stop" ]; then
    exit 3
...
exit 0
STUB
chmod +x "${FAILSTOP_AGENT_DIR}/bin/agent.sh"
printf 'serverUrl=https://teamcity.example.com\n' > "${FAILSTOP_AGENT_DIR}/conf/buildAgent.properties"
TEAMCITY_DAEMON_DRY_RUN_DIR="${FAILSTOP_WORK}/out" "$INSTALLER" \
    --agent-dir "$FAILSTOP_AGENT_DIR" --user "$AGENT_USER" --group "$AGENT_GROUP" > /dev/null

bash "${FAILSTOP_WORK}/out/teamcity-agent-service.sh" &
FAILSTOP_PID=$!
for _ in $(seq 1 200); do
    if grep -q '^start$' "$FAILSTOP_LOG" 2>/dev/null; then
        break
    fi
    sleep 0.1
done
kill -TERM "$FAILSTOP_PID" 2>/dev/null || true
set +e
wait "$FAILSTOP_PID"
failstop_status=$?
set -e
check "wrapper attempts the stop even when the agent is out of sync" \
    "$(grep -q '^stop force$' "$FAILSTOP_LOG" && echo true || echo false)"
check "wrapper exits 0 when the stop fails, so launchd does not respawn the agent" \
    "$([ "$failstop_status" -eq 0 ] && echo true || echo false)"
