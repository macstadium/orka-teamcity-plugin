...
stop_agent() {
    ...
    # Best-effort: under set -e a non-zero exit here (stale PID, agent already gone) would
    # skip the exit 0 below and hand launchd a failed exit, which KeepAlive would respawn.
    "\$AGENT" stop force || echo "agent.sh stop force failed, exiting anyway" >&2
    # The stop command only sends a request. Keep the wrapper alive until the launcher
    # exits, otherwise launchd signals the process group during agent unregistration.
    # Capture the PID before stopping: shutdown can remove the PID file before exiting.
    # launchd's ExitTimeOut bounds this wait if the agent cannot shut down.
...
# The agent-side plugin tells the two setup modes apart by whether this file exists: SSH mode
# uploads one before it starts the agent, so in daemon mode any copy present is a leftover from
# the VM the image was captured from, and using it would register every clone under that VM's
# instance id. Clearing it here rather than at install time means an image edited afterwards
# cannot reintroduce it. Best-effort, since under set -e a file another user owns in a sticky
# /tmp would otherwise take the daemon down.
rm -f /tmp/orka_metadata_file* "${AGENT_DIR}"/orka_metadata_file* || true
